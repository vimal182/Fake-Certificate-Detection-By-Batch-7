...
class RecruiterLogin(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Recruiter Login")
    ...
    def login(self):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT * FROM recruiter_users WHERE username=? AND password=?",
        ...
        if user:
            from recruiter_dash import RecruiterDash
            self.dashboard = RecruiterDash()
            self.dashboard.show()
            self.close()
        else:
    ...
    def open_signup(self, event):
        from recruiter_signup import RecruiterSignup
        self.signup = RecruiterSignup()
        self.signup.show()
        self.close()
